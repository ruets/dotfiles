Create Codex pet row `running` for `mini-ruets`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Create a compact whole-body chibi human pet inspired by the reference photo: reddish auburn side-swept hair, warm brown eyes, thick brows, light moustache and beard, black shirt, and a small black dangling cross earring on the left ear. Cute friendly Codex desktop companion, readable at 192x208.. Preserve silhouette, face, palette, material, proportions, markings, and props.
